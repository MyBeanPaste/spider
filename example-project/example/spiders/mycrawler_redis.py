...
class MyCrawler(RedisCrawlSpider):
    ...
    rules = (
        ...
        Rule(LinkExtractor(allow=r'search.51job.com/list/.*.html'), follow=True),
        Rule(LinkExtractor(allow=r'jobs.51job.com/\w+/\d+.html?'), callback='parse_item', follow=False),
    )
    ...
    def parse_page(self, response):
        ...
        if '招' in job_degree:
            job_degree = ''
            job_people = response.xpath('//div[@class="t1"]/span[@class="sp4"][2]/text()').extract()[0].lstrip(
                '招').rstrip('人')  # 招聘人数
        else:
            job_people = response.xpath('//div[@class="t1"]/span[@class="sp4"][3]/text()').extract()[0].lstrip(
                '招').rstrip('人')  # 招聘人数
        ...
        if job_degree == '':
            job_date_pub = response.xpath('//div[@class="t1"]/span[@class="sp4"][3]/text()').extract()[0].replace('发布',
                                                                                                                  '')  # 发布时间
        else:
            job_date_pub = response.xpath('//div[@class="t1"]/span[@class="sp4"][4]/text()').extract()[0].replace('发布',
                                                                                                                  '')  # 发布时间
